...
pub use ifa_types::capability::{CapabilitySet, Ofun};
...
impl Interpreter {
    ...
    fn import_module(&mut self, path: &[String]) -> IfaResult<IfaValue> {
        ...
        // R3: Enforce filesystem sandbox for module imports
        self.check_capability(&Ofun::ReadFiles { root: file_path.clone() })?;
    ...
    fn execute_statement(&mut self, stmt: &Statement) -> IfaResult<IfaValue> {
        ...
        let res = match stmt {
                ...
                let new_val = match op {
                    ...
                    _ => {
                        ...
                        let bin_op = match op {
                            ...
                            UpdateOp::ModAssign => BinaryOperator::Mod,
            ...
            Statement::While {
                ...
                'while_loop: while self.evaluate(condition)?.is_truthy() {
                    for s in body {
                        let res = self.execute_statement(s)?;
                        if res.is_return() {
                            return Ok(res);
                        ...
                        if res.is_continue() {
                            break; // break inner for-loop, restart while condition
                        }
                    }
                }
                Ok(IfaValue::Null)
            ...
            Statement::For {
                ...
                if let IfaValue::List(items) = iter_val {
                    ...
                    'for_loop: for item in items_vec {
                        ...
                        for s in body {
                            ...
                            if res.is_continue() {
                                break; // restart the for-item iteration
                            }
                        }
                    }
                }
                ...
                if let Some(names) = names {
                    for name in names {
                        let val = match &exports {
                            IfaValue::Map(map) => {
                                let key = ifa_types::CompactString::new(name.as_str());
            ...
            // K1: break/continue — signal consumed by nearest enclosing While/For
            Statement::Break { .. } => Ok(IfaValue::Break),
            Statement::Continue { .. } => Ok(IfaValue::Continue),
        };
            ...
            if !matches!(e, IfaError::Yielded) {
                ...
                if !err_str.contains(" [at ") {
                    ...
                    let file_name = self.current_file
                        ...
                        .unwrap_or_else(|| "unknown.ifa".to_string());
                    let loc = format!(" [at {}:{}]", file_name, line);
                    let e = e.clone();
                    let enriched_error = match e {
        ...
        match expr {
            ...
            Expression::BinaryOp { left, op, right } => {
                ...
                match op {
                    ...
                    // ?? short-circuit: eval lhs; if non-null return it, else eval rhs
            ...
            // K2: Lambda — evaluate to a callable AstFn value capturing current scope.
            Expression::Lambda { params, body } => {
                let closure_id = self.next_closure_id;
                self.next_closure_id = self.next_closure_id.saturating_add(1);
                self.closures.insert(closure_id, self.env.clone());
                Ok(IfaValue::AstFn(Arc::new(ifa_types::value_union::AstFnData {
    ...
    fn apply_binary_op(
        ...
        match op {
            ...
            // G4: null coalescing — value-level fallback (short-circuit handled in evaluate())
            BinaryOperator::NullCoalesce => Ok(if !left.is_null() {
                left.clone()
            } else {
                right.clone()
            }),
            ...
            BinaryOperator::Power => match (left, right) {
                (IfaValue::Int(a), IfaValue::Int(b)) if *b >= 0 => {
                    Ok(IfaValue::int(i64::pow(*a, *b as u32)))
                }
                (IfaValue::Float(a), IfaValue::Float(b)) => Ok(IfaValue::float(a.powf(*b))),
                (IfaValue::Int(a), IfaValue::Float(b)) => Ok(IfaValue::float((*a as f64).powf(*b))),
                (IfaValue::Float(a), IfaValue::Int(b)) => Ok(IfaValue::float(a.powi(*b as i32))),
                _ => Err(IfaError::Runtime("Invalid operands for **".into())),
            },
        }
    }
    ...
    fn call_method(
        ...
        match obj {
            IfaValue::Map(map) => {
                let key = ifa_types::CompactString::new(method);
...
fn odu_domain_from_id(id: u8) -> Option<OduDomain> {
    match id {
        ...
        // Reserved pseudo-domains (no active dispatch)
...
// ============================================================================
// Interpreter Callback / Functional Primitives Support
// ============================================================================

use std::cell::RefCell;

thread_local! {
    static INTERPRETER_CALLBACK: RefCell<Option<*mut Interpreter>> = RefCell::new(None);
}

pub(crate) struct CallbackGuard {
    old: Option<*mut Interpreter>,
}

impl CallbackGuard {
    pub fn new(interp: &mut Interpreter) -> Self {
        let old = INTERPRETER_CALLBACK.with(|cell| cell.replace(Some(interp as *mut Interpreter)));
        CallbackGuard { old }
    }
}
...
impl Drop for CallbackGuard {
    fn drop(&mut self) {
        INTERPRETER_CALLBACK.with(|cell| {
            cell.replace(self.old);
        });
    }
}

/// Evaluates a callback function/closure on the active interpreter instance.
/// 
/// SAFETY: The callback is invoked on the thread-local active `Interpreter` pointer.
/// This pointer is guaranteed to be valid during the lifetime of `CallbackGuard`,
/// which wraps the OduHandler dispatch.
pub(crate) fn evaluate_callback(func: &IfaValue, args: Vec<IfaValue>) -> IfaResult<IfaValue> {
    INTERPRETER_CALLBACK.with(|cell| {
        if let Some(ptr) = *cell.borrow() {
            ...
            Err(IfaError::Runtime("Interpreter callback context not set".into()))
        }
    })
}

impl Interpreter {
    /// Dispatches a method call to the modular domain handlers with a CallbackGuard set.
    pub fn dispatch_odu(
        &mut self,
        domain: OduDomain,
        method: &str,
        ...
        let _guard = CallbackGuard::new(self);
    ...
    /// Evaluates/calls a functional value (closure, function, or string marker)
    pub fn call_value(&mut self, func: &IfaValue, args: Vec<IfaValue>) -> IfaResult<IfaValue> {
        match func {
            IfaValue::AstFn(data) => {
                let env = self.closures.get(&data.closure_id).cloned().ok_or_else(|| {
                    IfaError::Runtime("Closure environment missing".into())
                })?;
                self.call_ast_function_values(&data.params, &data.body, env, args)
            }
            IfaValue::Str(s) => {
                if let Some((domain, method)) = parse_odu_fn_marker(&s) {
                    self.dispatch_odu(domain, &method, args)
                } else {
                    Err(IfaError::TypeError {
                        expected: "Function".into(),
                        got: "Str".into(),
