...
//!
//! Handles HTTP and WebSocket operations. Gated by Esu and standard library Otura SSRF protection.
//! Binary pattern: 1011
...
impl OduHandler for OturaHandler {
    ...
    fn call(
        ...
        #[cfg(feature = "std")]
        {
            ...
            let otura = ifa_std::odu::otura::Otura::new(esu);

            let arg0 = args.first();
            let arg1 = args.get(1);

            let check_ssrf = |url: &str| -> IfaResult<()> {
                if let Some(host) = url.split("://").nth(1).and_then(|s| s.split('/').next()) {
                    let host = host.split(':').next().unwrap_or(host);
                    if !otura.ṣàyẹ̀wò(host) {
                        return Err(IfaError::Runtime(format!(
                            "SSRF or Capability check failed for host: {}",
                            host
                        )));
                    }
                    Ok(())
                } else {
                    Err(IfaError::Runtime("Invalid URL".into()))
                }
            };
            ...
            match method {
                ...
                _ => Err(IfaError::Runtime(format!("Unknown Òtúrá method: {}", method))),
            }
        }
        #[cfg(not(feature = "std"))]
        {
