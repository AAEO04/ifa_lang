        ...
        match method {
            ...
            "ni_agbara" | "has_capability" | "can" => {
                if let Some(IfaValue::Str(cap)) = arg0 {
                    ...
                    let has_cap = match cap_type {
                        "Stdio" => _capabilities.check(&ifa_types::capability::Ofun::Stdio),
                        "Time" => _capabilities.check(&ifa_types::capability::Ofun::Time),
                        "Random" => _capabilities.check(&ifa_types::capability::Ofun::Random),
                        "ReadFiles" => _capabilities.all().iter().any(|c| matches!(c, ifa_types::capability::Ofun::ReadFiles { .. } | ifa_types::capability::Ofun::WriteFiles { .. })),
                        "Network" => _capabilities.all().iter().any(|c| matches!(c, ifa_types::capability::Ofun::Network { .. })),
                        "Environment" => _capabilities.all().iter().any(|c| matches!(c, ifa_types::capability::Ofun::Environment { .. })),
                        "Execute" => _capabilities.all().iter().any(|c| matches!(c, ifa_types::capability::Ofun::Execute { .. })),
                        "Bridge" => _capabilities.all().iter().any(|c| matches!(c, ifa_types::capability::Ofun::Bridge { .. })),
                        _ => false,
                    };
            ...
            // Request capability (logs and returns success if already granted)
            "beere" | "request" => {
                if let Some(IfaValue::Str(cap)) = arg0 {
                    let cap_type = parse_capability_name(cap);
                    if cap_type == "Unknown" {
