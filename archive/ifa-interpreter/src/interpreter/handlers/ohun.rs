...
fn play_audio_file(path: &str) -> IfaResult<bool> {
    Err(IfaError::Runtime(format!(
        "Audio disabled. Enable 'audio' feature to play files. Path: {}",
        path
    )))
}
        ...
        match method {
            ...
            "ṣe" | "play" => {
                if let Some(IfaValue::Str(path)) = args.first() {
                    let canon_path = ifa_types::capability::canonicalize_safe(std::path::Path::new(path));
                    if !_capabilities.check(&ifa_types::capability::Ofun::ReadFiles { root: canon_path }) {
                        return Err(IfaError::PermissionDenied(format!(
                            "Permission denied: play requires read capability for '{}'",
                            path
                        )));
                    }
            ...
            "gba" | "record" => Err(IfaError::Runtime(
