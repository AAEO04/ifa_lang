# Ifa-Lang Runtime Specification

**Version:** 0.2 Unified Canonical Specification  
...
|:---|:---:|:---:|:---:|:---:|
| Integer arithmetic | Y | Y | Y | Y |
| Float arithmetic | Y | Y | Y | Y |
| String literals | Y | Y | Y | N |
| String interpolation | N | N | N | N |
| Boolean logic | Y | Y | Y | Y |
| Comparison operators | Y | Y | Y | Y |
| Variable declaration | Y | Y | Y | Y |
| Variable shadowing | Y | Y | Y | Y |
| Conditionals | Y | Y | Y | Y |
| While loops | Y | Y | Y | Y |
| For loops | P | Y | P | N |
| Break / continue | N | N | N | N |
| Pattern matching (`yan`) | Y | Y | Y | N |
| User-defined functions | Y | Y | Y | N |
| Default parameters | N | N | N | N |
| Variadic parameters | N | N | N | N |
...
| Map literals | Y | Y | Y | N |
| Optional chaining | N | N | N | N |
| Null coalescing | N | N | N | N |
| Standard domain calls | Y | P | P | N |
| Error handling (`gbiyanju`) | P | P | N | N |
| Error propagation (`?`) | N | Y | N | N |
| Module system (`mu` / `fi`) | Y | Y | P | N |
| Async / await | Y | P | P | N |
| Channels | N | N | N | N |
| Pointer / mmap operations | Y | Y | N | Y |
| Yield | Y | Y | Y | Y |
| Sized integer types | N | N | N | Y |
| Tail-call optimization | N | Y | N | N |
| REPL | Y | N | N | N |
| Cryptography (Ìrẹtẹ̀) | Y | Y | Y | N |

**Legend:** `Y` Working · `P` Partial · `N` Not implemented today
...
### 25.4 Dynamic Observation (Maps)

Maps are inherently observable through the Àjọṣe registry. A reactive binding can be placed on any field of any Map.

```
ayanmo c = { value: 0 };
ajose!(c.value => #print);  # hypothetical convenience for debugging
c.value = 5;                # triggers the binding
```
...
### 26.1 The `#[derive(Ebo)]` Attribute (Struct Transformation)

The `#[derive(Ebo)]` attribute can be applied to Map-like structures or internal Rust-backed resources to generate `Drop` logic.

```
#[derive(Ebo)]
#[ebo(cleanup = "close")]
# Managed resource stub
ese open_file(path) { ... }
```
...
```
ayanmo results = reti Osa.gbogbo([
...
| `DefineClass` | *"The odu has been retired. The oracle now uses maps."* |
