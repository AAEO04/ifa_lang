...
use crate::wisdom::{ODU_WISDOM, ERROR_TO_ODU};
use std::fmt;
...
impl Babalawo {
    ...
    fn add_diagnostic(&mut self, severity: Severity, code: &str, msg: &str, 
    ...
    pub fn format(&self) -> String {
        ...
        for diag in &self.diagnostics {
            let odu_name = ODU_WISDOM.get(diag.odu.as_str())
                .map(|w| w.name)
                .unwrap_or("Ọ̀kànràn");
...
mod tests {
    ...
    fn test_babalawo_error() {
        ...
        baba.error("UNDEFINED_VARIABLE", "Variable 'x' not defined", "test.ifa", 10, 5);
    ...
    fn test_babalawo_warning() {
        let mut baba = Babalawo::new();
        baba.warning("UNUSED_VARIABLE", "Variable 'temp' is unused", "test.ifa", 5, 10);
    ...
    fn test_compact_format() {
        let mut baba = Babalawo::new();
        baba.error("DIVISION_BY_ZERO", "Cannot divide by zero", "math.ifa", 25, 15);
