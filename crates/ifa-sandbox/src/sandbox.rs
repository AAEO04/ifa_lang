...
impl Sandbox {
    ...
    pub fn can_access_file(&self, path: &Path) -> bool {
        ...
        self.config.capabilities.check(&Ofun::ReadFiles { root: canonical })
    }
    ...
    pub fn can_create_file(&self, _path: &Path) -> bool {
        ...
        self.config.capabilities.all().iter().any(|c| matches!(c, Ofun::WriteFiles { .. }))
    }
    
    /// Check if process spawning is allowed
    pub fn can_spawn_process(&self) -> bool {
        self.config.capabilities.all().iter().any(|c| matches!(c, Ofun::Execute { .. }))
    }
