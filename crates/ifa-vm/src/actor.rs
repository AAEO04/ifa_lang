    ...
    pub fn send(&self, value: IfaValue) -> IfaResult<()> {
        self.tx
            ...
            .map_err(|e| match e {
                ...
                TrySendError::Disconnected(_) => IfaError::Runtime(format!(
                    "Actor {} has exited",
                    self.id
                )),
            })
...
pub fn spawn_actor(
    ...
    let tx = Arc::new(tx);
    let handle = ActorHandle { id, tx: tx.clone(), resource_registry: resource_registry.clone() };
    table.insert(handle.clone());

    let table_clone = table.clone();
    spawn_actor_task(move || {
        actor_loop(id, init_fn, rx, &bytecode, &table_clone, registry, resource_registry);
    });
...
pub fn transfer_resources(value: &IfaValue, sender_registry: &ifa_types::ResourceRegistry, recipient_registry: &ifa_types::ResourceRegistry, recipient_actor_id: u64) {
    match value {
        ...
        IfaValue::Result(payload) => {
            match payload.as_ref() {
                ifa_types::value_union::ResultPayload::Ok(v) => transfer_resources(v, sender_registry, recipient_registry, recipient_actor_id),
                ifa_types::value_union::ResultPayload::Err(v) => transfer_resources(v, sender_registry, recipient_registry, recipient_actor_id),
            }
        }
        _ => {}
    }
...
pub fn actor_send(actor: &IfaValue, value: IfaValue, sender_registry: &ifa_types::ResourceRegistry) -> IfaResult<()> {
        ...
        let safe_value = match value.freeze() {
            ...
            Err(_) => {
                return Err(IfaError::Runtime(format!(
                    "Cannot send thread-unsafe value to actor: {}", value.type_name()
                )));
            }
        };

        // 2. Transfer ownership of any resources contained in the message payload
        transfer_resources(&safe_value, sender_registry, &actor_handle.resource_registry, *id);

        actor_handle.tx
            .try_send(ActorMsg::Value(safe_value))
            .map_err(|e| match e {
                mpsc::TrySendError::Full(_) => IfaError::Runtime(format!(
                    "Actor {} inbox full — back-pressure required", id
                )),
                mpsc::TrySendError::Disconnected(_) => IfaError::Runtime(format!(
                    "Actor {} has exited", id
                )),
            })
    } else {
...
mod tests {
    ...
    fn actor_table_insert_get_remove() {
        ...
        let handle = ActorHandle { id: 999, tx: Arc::new(tx), resource_registry: registry };
        table.insert(handle.clone());
    ...
    fn send_to_disconnected_actor_errors() {
        ...
        drop(_rx);
        let registry = Arc::new(ifa_types::registry::ResourceRegistry::new());
        let handle = ActorHandle { id: 1, tx: Arc::new(tx), resource_registry: registry };
