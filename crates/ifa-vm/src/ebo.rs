...
impl<F: FnOnce()> Ebo<F> {
    ...
    pub fn dismiss(mut self) {
        // SAFETY: We are taking ownership of the inner value and dropping it immediately.
        // We then forget `self` so that `Drop::drop` is not called, preventing a double free.
        unsafe {
    ...
    pub fn sacrifice(mut self) {
        // SAFETY: We take the inner value to call it.
        // `self` is then forgotten, so `Drop::drop` will not run and we avoid double-taking/double-freeing.
...
impl<F: FnOnce()> Drop for Ebo<F> {
    #[inline]
    fn drop(&mut self) {
        // SAFETY: `Drop::drop` is only called once. If `dismiss` or `sacrifice` were called,
        // `self` would have been forgotten and this code never reached.
        if let Some(cleanup) = unsafe { ManuallyDrop::take(&mut self.cleanup) } {
            cleanup();
        }
    }
