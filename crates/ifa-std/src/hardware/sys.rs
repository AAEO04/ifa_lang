...
pub fn dispatch(method: &str, _args: Vec<IfaValue>) -> IfaResult<IfaValue> {
    match method {
        ...
        #[cfg(feature = "sysinfo")]
        "total_memory" | "mem_total" => Ok(IfaValue::int(
            ifa_infra::kernel::total_memory() as i64,
        )),
        #[cfg(feature = "sysinfo")]
        "available_memory" | "mem_available" => Ok(IfaValue::int(
            ifa_infra::kernel::available_memory() as i64,
        )),
        #[cfg(feature = "sysinfo")]
        ...
        "total_memory" | "mem_total" | "available_memory" | "mem_available" | "uptime" => {
            Err(IfaError::Custom(
                "Sys: sysinfo feature not enabled".into(),
            ))
        }
        _ => Err(IfaError::Custom(format!("Sys: unknown method '{}'", method))),
    }
}
