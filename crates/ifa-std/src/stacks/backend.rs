...
impl Middleware {
    /// Logging middleware
    pub fn logger() -> Self {
        Middleware { name: "logger".to_string() }
    }
    
    /// CORS middleware
    pub fn cors(origins: &[&str]) -> Self {
        println!("[Middleware] CORS: {:?}", origins);
        Middleware { name: "cors".to_string() }
    }
    
    /// Rate limiting
    pub fn rate_limit(max_requests: u32, window_secs: u32) -> Self {
        println!("[Middleware] RateLimit: {} req/{} sec", max_requests, window_secs);
        Middleware { name: "ratelimit".to_string() }
    }
    
    /// Authentication
    pub fn auth() -> Self {
        Middleware { name: "auth".to_string() }
    }
}
