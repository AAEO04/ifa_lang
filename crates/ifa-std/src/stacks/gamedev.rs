...
impl std::ops::Add for Vec2 {
    type Output = Self;
    fn add(self, other: Self) -> Self { Self::add(self, other) }
}

impl std::ops::Sub for Vec2 {
    type Output = Self;
    fn sub(self, other: Self) -> Self { Self::sub(self, other) }
}

impl std::ops::Mul<f32> for Vec2 {
    type Output = Self;
    fn mul(self, s: f32) -> Self { self.scale(s) }
}
...
impl Entity {
    pub fn new(id: u64) -> Self { Entity(id) }
}
...
impl World {
    ...
    pub fn query_tagged(&self, tag: &str) -> Vec<Entity> {
        self.entities.iter()
            .filter(|e| self.tags.get(e).map(|t| t.contains(&tag.to_string())).unwrap_or(false))
            .copied()
            .collect()
    }
...
impl GameTimer {
    pub fn once(duration: f32) -> Self {
        GameTimer { duration, elapsed: 0.0, repeating: false, finished: false }
    }
    
    pub fn repeating(duration: f32) -> Self {
        GameTimer { duration, elapsed: 0.0, repeating: true, finished: false }
    }
