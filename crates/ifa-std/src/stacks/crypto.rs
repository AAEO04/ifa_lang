...
pub mod base64 {
    ...
    pub fn decode(encoded: &str) -> Result<Vec<u8>, CryptoError> {
        ...
        for chunk in chars.chunks(4) {
            ...
            let indices: Result<Vec<usize>, _> = chunk.iter()
                .map(|&c| {
                    ALPHABET.iter().position(|&b| b as char == c)
                        .ok_or_else(|| CryptoError::InvalidInput(format!("Invalid base64 char: {}", c)))
                })
                .collect();
...
pub mod hex {
    ...
    pub fn decode(encoded: &str) -> Result<Vec<u8>, CryptoError> {
        if encoded.len() % 2 != 0 {
            return Err(CryptoError::InvalidInput("Odd length hex string".to_string()));
        }
...
impl SecretStore {
    pub fn new() -> Self {
        SecretStore { secrets: HashMap::new() }
    }
