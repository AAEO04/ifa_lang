...
impl Ika {
    fn ifa_to_json_value(val: &IfaValue) -> IfaResult<serde_json::Value> {
        match val {
            IfaValue::Null => Ok(serde_json::Value::Null),
            IfaValue::Bool(b) => Ok(serde_json::Value::Bool(*b)),
            IfaValue::Int(i) => Ok(serde_json::Value::Number((*i).into())),
            IfaValue::Float(f) => serde_json::Number::from_f64(*f)
                .map(serde_json::Value::Number)
                .ok_or_else(|| IfaError::Custom("JSON error: non-finite float".into())),
            IfaValue::Str(s) => Ok(serde_json::Value::String(s.to_string())),
            IfaValue::List(items) => {
                let mut out = Vec::with_capacity(items.len());
                for item in items.iter() {
                    out.push(Self::ifa_to_json_value(item)?);
                }
            ...
            IfaValue::Map(map) => {
                ...
                for (key, value) in map.iter() {
                    out.insert(key.to_string(), Self::ifa_to_json_value(value)?);
                }
                Ok(serde_json::Value::Object(out))
            }
            other => Err(IfaError::Custom(format!(
                "JSON error: {} is not serializable",
                other.type_name()
            ))),
        }
    }

    fn json_value_to_ifa(value: serde_json::Value) -> IfaValue {
        match value {
            ...
            serde_json::Value::Object(obj) => {
                ...
                for (key, value) in obj {
                    map.insert(key, Self::json_value_to_ifa(value));
                }
                IfaValue::map(map)
            }
        }
    ...
    pub fn yi_si_json(&self, val: &IfaValue) -> IfaResult<String> {
        let json = Self::ifa_to_json_value(val)?;
        serde_json::to_string(&json).map_err(|e| IfaError::Custom(format!("JSON error: {}", e)))
    }
    ...
    pub fn tumo_html(&self, html: &str) -> String {
        ...
        for node in dom.nodes() {
            if let Some(t) = node.as_raw() {
                text.push_str(&t.as_utf8_str());
            }
        }
        text
    }
