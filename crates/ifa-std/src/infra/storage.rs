...
impl TryFrom<u8> for RecordType {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        match value {
            ...
            _ => Err(StorageError::CorruptedLog(format!("Invalid record type: {}", value))),
        }
    }
}
...
impl OduStore {
    ...
    async fn rebuild_index(
        file: Arc<Mutex<File>>
    ) -> Result<(HashMap<String, u64>, std::collections::HashSet<String>, usize, usize), StorageError> {
        ...
        while pos < file_len {
            ...
            if key_len > MAX_KEY_SIZE {
                return Err(StorageError::CorruptedLog(
                    format!("Invalid key length {} at offset {}", key_len, record_start)
                ));
            }
            ...
            let val_bytes = if record_type == RecordType::Put {
                ...
                if val_len > MAX_VALUE_SIZE {
                    return Err(StorageError::CorruptedLog(
                        format!("Invalid value length {} at offset {}", val_len, record_start)
                    ));
                }
    ...
    async fn write_record(&mut self, record_type: RecordType, key: &str, value: Option<&[u8]>) -> Result<u64, StorageError> {
    ...
    pub async fn set<V: Serialize>(&mut self, key: &str, value: &V) -> Result<(), StorageError> {
        ...
        let offset = self.write_record(RecordType::Put, key, Some(&serialized)).await?;
