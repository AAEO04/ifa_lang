...
mod cross_domain_tests {
    ...
    fn test_network_file_download() {
        #[cfg(feature = "full")]
        {
            ...
            let response = tokio::task::block_on(async {
                otura.gba("https://httpbin.org/uuid").await
            });
    ...
    fn test_crypto_file_operations() {
        ...
        odi.kowe(&encrypted_file, &String::from_utf8_lossy(&encrypted)).unwrap();
        
        // Decrypt the file
        let encrypted_data = odi.kawe(&encrypted_file).unwrap();
        let decrypted = irete.decrypt(encrypted_data.as_bytes(), key.as_bytes());
        odi.kowe(&decrypted_file, &String::from_utf8_lossy(&decrypted)).unwrap();
    ...
    fn test_network_error_handling() {
        #[cfg(feature = "full")]
        {
            ...
            let result = tokio::task::block_on(async {
                otura.gba("http://invalid-host-that-does-not-exist.com").await
            });
