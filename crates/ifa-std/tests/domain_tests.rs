...
mod oturupon_tests {
    ...
    fn test_rounding_modes() {
        ...
        assert_eq!(oturupon.pin_f(7.0, 2.0, RoundingMode::Ceiling).unwrap(), 4.0);
        assert_eq!(oturupon.pin_f(7.0, 2.0, RoundingMode::Truncate).unwrap(), 3.0);
    }
}
...
mod iwori_tests {
    use super::*;
    use ifa_std::iwori::{Iwori, range, range_step, repeat};
...
mod odi_tests {
    ...
    fn test_sqlite() {
        ...
        conn.execute("INSERT INTO test (name) VALUES (?)", ["Ifá"]).unwrap();
...
mod okanran_tests {
    ...
    fn test_gbiyanju_with_error() {
        ...
        let result = okanran.gbiyanju(
            || Err(IfaError::Custom("test error".into())),
            42
        );
...
mod ofun_tests {
    use super::*;
    use ifa_std::ofun::{Ofun, Capabilities};
