...
impl ComputeBackend for GpuContext {
    ...
    fn matmul(&self, a: &[f32], b: &[f32], m: usize, n: usize, k: usize) -> Vec<f32> {
        ...
        let bytes = self
            .read_buffer(&buf_c)
            .unwrap_or_else(|_| vec![0u8; m * n * std::mem::size_of::<f32>()]);
...
use ifa_embedded::slab::{SlabAllocation, SlabClass, SlabTracker};
...
impl Slab {
    fn new(device: &Device, slot_size: u64, slot_count: usize) -> Self {
        ...
        Self {
            buffer,
            tracker: SlabTracker::new(slot_size, slot_count),
        }
    }
