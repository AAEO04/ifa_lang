...
impl RustTranspiler {
    ...
    pub fn transpile_expression(&mut self, expr: &Expression) -> String {
        match expr {
            ...
            // K2: Lambda — transpile as a Rust closure (best-effort; no full closure env)
            Expression::Lambda { params, body } => {
                let params_str: Vec<String> = params.iter().map(|p| format!("{}: IfaValue", p)).collect();
                let mut inner = String::new();
                for s in body {
                    inner.push_str(&self.transpile_statement(s));
                    inner.push('\n');
                }
                format!("(|{}| -> IfaValue {{ {}; IfaValue::Nil }})", params_str.join(", "), inner.trim())
            }
        }
    }
}
