...
impl RustTranspiler {
    ...
    pub fn transpile_odu_call(&mut self, call: &OduCall) -> String {
        ...
        match domain_str.as_str() {
            ...
            // ═══════════════════════════════════════════════════════════════════
            // ÒGÚNDÁ (1110) - Arrays/Lists
            // ═══════════════════════════════════════════════════════════════════
            "ogunda" if method == "da" || method == "create" || method == "new" => {
                format!("IfaValue::List(std::sync::Arc::new(vec![{}]))", args.join(", "))
            }
            "ogunda" if matches_method(&method, ogunda::LENGTH) || method == "iwon" => {
                if let Some(list) = args.first() {
                    format!("IfaValue::Int(match {} {{ IfaValue::List(l) => l.len() as i64, _ => 0 }})", list)
                } else {
                    "IfaValue::Int(0)".to_string()
                }
            }
            "ogunda" if matches_method(&method, ogunda::APPEND) || method == "fi" => {
                if args.len() >= 2 {
                    format!("IfaValue::list({{ let mut l = match {} {{ IfaValue::List(inner) => inner.as_ref().clone(), _ => vec![] }}; l.push({}.clone()); l }})", args[0], args[1])
                } else {
                    "IfaValue::Nil".to_string()
                }
            }
            "ogunda" if method == "pop" || method == "mu" || method == "yọ" => {
                if let Some(list) = args.first() {
                    format!("match {} {{ IfaValue::List(inner) => inner.last().cloned().unwrap_or(IfaValue::Nil), _ => IfaValue::Nil }}", list)
                } else {
                    "IfaValue::Nil".to_string()
                }
            }
            "ogunda" if method == "map" || method == "yi_pada" || method == "yipada" || method == "maapu" => {
                if args.len() >= 2 {
                    format!("IfaValue::list({{ let l = match {} {{ IfaValue::List(inner) => inner.as_ref().clone(), _ => vec![] }}; let f = {}; l.into_iter().map(f).collect() }})", args[0], args[1])
                } else {
                    "IfaValue::Nil".to_string()
                }
            }
            "ogunda" if method == "filter" || method == "yan" || method == "sajo" || method == "ṣàjọ" => {
                if args.len() >= 2 {
                    format!("IfaValue::list({{ let l = match {} {{ IfaValue::List(inner) => inner.as_ref().clone(), _ => vec![] }}; let f = {}; l.into_iter().filter(|x| f(x.clone()).is_truthy()).collect() }})", args[0], args[1])
                } else {
                    "IfaValue::Nil".to_string()
                }
            }
            "ogunda" if method == "reduce" || method == "seku" || method == "ṣẹ́kù" || method == "fold" || method == "din" => {
                if args.len() == 2 {
                    format!("{{ let l = match {} {{ IfaValue::List(inner) => inner.as_ref().clone(), _ => vec![] }}; let f = {}; if l.is_empty() {{ IfaValue::Nil }} else {{ let mut acc = l[0].clone(); for item in l.into_iter().skip(1) {{ acc = f(acc, item); }} acc }} }}", args[0], args[1])
                } else if args.len() >= 3 {
                    format!("{{ let l = match {} {{ IfaValue::List(inner) => inner.as_ref().clone(), _ => vec![] }}; let f = {}; let mut acc = {}; for item in l {{ acc = f(acc, item); }} acc }}", args[0], args[2], args[1])
                } else {
                    "IfaValue::Nil".to_string()
                }
            }
