...
use ifa_core::interpreter::{Interpreter, Debugger, Environment, CapabilitySet, Ofun};
    ...
    fn wait_for_command(&self) {
        ...
        loop {
            ...
            if header.starts_with("Content-Length: ") {
                ...
                if let Ok(msg) = serde_json::from_str::<ProtocolMessage>(&body) {
    ...
    fn handle_request(&self, req: Request) {
        ...
        match req.command.as_str() {
            ...
            "launch" => {
                ...
                self.send_event("process".to_string(), Some(serde_json::json!({
                    "name": "ifa-debug",
...
impl Debugger for DapAdapter {
    fn on_statement(&mut self, stmt: &Statement, env: &Environment) {
        // extract location
        let (line, file) = match stmt {
             Statement::VarDecl { span, .. } | Statement::Assignment { span, .. } |
             Statement::Instruction { span, .. } | Statement::If { span, .. } |
             Statement::While { span, .. } | Statement::For { span, .. } |
             Statement::Return { span, .. } | Statement::Expr { span, .. } |
             Statement::Import { span, .. } | Statement::Ebo { span, .. } |
             Statement::Ewo { span, .. } | Statement::Opon { span, .. } |
             Statement::Taboo { span, .. } | Statement::Match { span, .. } |
             Statement::Ase { span }
             => (span.line, "unknown.ifa"), // TODO: Span needs file path? Or assume single file for simple case
             _ => (0, ""),
        };
        ...
        if should_stop {
            *self.paused.lock().unwrap() = true;
            ...
            // Send Stopped Event
            self.send_event("stopped".to_string(), Some(serde_json::json!({
...
pub fn run_debug_session(file: std::path::PathBuf) -> color_eyre::Result<()> {
    ...
    interpreter.execute(&program).map_err(|e| color_eyre::eyre::eyre!("Runtime error: {}", e))?;
