...
mod value_tests {
    ...
    fn test_string_negative_indexing() {
        ...
        assert_eq!(
            s.get(&IfaValue::Int(0)).unwrap(),
            IfaValue::Str("I".to_string())
        );
        assert_eq!(
            s.get(&IfaValue::Int(-1)).unwrap(),
            IfaValue::Str("á".to_string())
        );
    }
    ...
    fn test_list_slice() {
        ...
        let expected = IfaValue::List(vec![IfaValue::Int(2), IfaValue::Int(3), IfaValue::Int(4)]);
