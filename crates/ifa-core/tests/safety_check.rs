...
#[test]
// RETIRED: interpreter-level pointer deref is not supported in the current type system.
// Pointer semantics live in the bytecode VM (OpCode::Ref / OpCode::Load8 etc.).
// Re-enable when the interpreter is removed or IfaValue::Ptr is restored.
#[ignore]
fn test_safety_violation_outside_ailewu() {
    let mut interp = Interpreter::new();

    interp.opon.set(0x100, IfaValue::Int(42));
    Environment::define(&interp.env, "ptr", IfaValue::Int(0x100));
...
fn test_safety_compliance_inside_ailewu() {
    ...
    let val = Environment::get(&interp.env, "val");
...
fn test_unsafe_write_violation() {
    let mut interp = Interpreter::new();
    Environment::define(&interp.env, "ptr", IfaValue::Int(0x200));
