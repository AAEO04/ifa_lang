...
impl OduHandler for OturuponHandler {
        ...
        match method {
            ...
            "yọkuro" | "sub" | "subtract" => {
                match (args.first(), args.get(1)) {
                    (Some(IfaValue::Int(a)), Some(IfaValue::Int(b))) => {
                        Ok(IfaValue::Int(a - b))
                    }
                    (Some(IfaValue::Float(a)), Some(IfaValue::Float(b))) => {
    ...
    fn methods(&self) -> &'static [&'static str] {
        &["yọkuro", "sub", "subtract", "pin", "div", "divide", "iyoku", "mod", "modulo", "floor_div", "neg", "negate", "sqrt"]
    }
}
