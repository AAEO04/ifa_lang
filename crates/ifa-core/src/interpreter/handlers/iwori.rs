...
impl OduHandler for IworiHandler {
            ...
            "laarin" | "range" => {
                if args.len() >= 2 {
                    if let (IfaValue::Int(start), IfaValue::Int(end)) = (&args[0], &args[1]) {
                        let list: Vec<IfaValue> = (*start..*end)
                            .map(IfaValue::Int)
                            .collect();
                        return Ok(IfaValue::List(list));
                    }
                }
                Err(IfaError::Runtime("range requires start and end integers".into()))
            }
    ...
    fn methods(&self) -> &'static [&'static str] {
        &["bayi", "now", "timestamp", "bayi_ms", "now_ms", "aago", "elapsed", 
          "ṣe_ọjọ", "format", "iso", "ka_ọjọ", "parse", "laarin", "range"]
    }
}
