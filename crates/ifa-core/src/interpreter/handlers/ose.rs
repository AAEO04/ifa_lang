...
impl OduHandler for OseHandler {
    ...
    fn call(
        ...
        match method {
            ...
            "lọ_si" | "goto" | "move_to" => {
                ...
                Err(IfaError::Runtime("goto requires x and y coordinates".into()))
            }
            ...
            "apoti" | "box" => {
                if args.len() >= 4 {
                    if let (IfaValue::Int(x), IfaValue::Int(y), IfaValue::Int(w), IfaValue::Int(h)) = 
            ...
            "kọ_si" | "print_at" => {
                if args.len() >= 3 {
                    if let (IfaValue::Int(x), IfaValue::Int(y), text) = (&args[0], &args[1], &args[2]) {
    ...
    fn methods(&self) -> &'static [&'static str] {
        &["nu", "clear", "lọ_si", "goto", "move_to", "awọ", "color",
          "apoti", "box", "kọ_si", "print_at", "fia_kasọta", "hide_cursor",
          "ṣafihan_kasọta", "show_cursor", "iwọn", "size"]
    }
}
