    ...
    fn call(
        ...
        match method {
            ...
            "isoro_afiwe" | "parallel_product" | "product" => {
                ...
                Err(IfaError::Runtime("product requires a list of numbers".into()))
            }
            ...
            "tọ_afiwe" | "parallel_sort" | "sort" => {
                if let Some(IfaValue::List(list)) = args.first() {
                    ...
                    let mut indexed: Vec<(usize, f64)> = list.iter()
                        .enumerate()
                        .map(|(i, v)| {
                            let val = match v {
                    ...
                    let sorted: Vec<IfaValue> = indexed.iter()
                        .map(|(i, _)| list[*i].clone())
                        .collect();
            ...
            "bẹrẹ" | "spawn" | "duro" | "await" | "fi" | "send" | "gba" | "recv" => {
                Err(IfaError::Runtime(
                    "Async operations require tokio runtime. Use ifa-std with tokio feature.".into()
                ))
            }
