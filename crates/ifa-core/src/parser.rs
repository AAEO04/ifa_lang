...
pub fn parse(source: &str) -> IfaResult<Program> {
    let pairs = IfaParser::parse(Rule::program, source)
        .map_err(|e| IfaError::Parse(format!("{}", e)))?;
...
fn parse_statement(pair: pest::iterators::Pair<Rule>) -> IfaResult<Option<Statement>> {
    ...
    match pair.as_rule() {
        ...
        Rule::var_decl => {
            ...
            Ok(Some(Statement::VarDecl { name, type_hint, value, visibility: Visibility::default(), span }))
        }
        ...
        Rule::if_stmt => {
            ...
            Ok(Some(Statement::If { condition, then_body, else_body, span }))
        }
        ...
        Rule::while_stmt => {
            ...
            Ok(Some(Statement::While { condition, body, span }))
        }
        ...
        Rule::for_stmt => {
            ...
            Ok(Some(Statement::For { var, iterable, body, span }))
        }
        ...
        Rule::taboo_stmt => {
            ...
            Ok(Some(Statement::Taboo { source, target, span }))
        }
        ...
        Rule::ewo_stmt => {
            ...
            Ok(Some(Statement::Ewo { condition, message, span }))
        }
        ...
        Rule::ese_def => {
                ...
                match p.as_rule() {
                    Rule::params => {
                        for param_pair in p.into_inner() {
                            ...
                            params.push(Param { name: param_name, type_hint: param_type });
                        }
                    }
                    Rule::statement => {
                        if let Some(stmt) = parse_statement(p)? {
            ...
            Ok(Some(Statement::EseDef { name, visibility, params, body, span }))
        }
        ...
        Rule::odu_def => {
            ...
            Ok(Some(Statement::OduDef { name, visibility, body, span }))
        }
...
fn parse_expression(pair: pest::iterators::Pair<Rule>) -> IfaResult<Expression> {
    match pair.as_rule() {
        Rule::expression | Rule::or_expr | Rule::and_expr | Rule::not_expr 
        | Rule::comparison | Rule::arith_expr | Rule::term | Rule::factor => {
            let mut inner = pair.into_inner();
            let first = inner.next().unwrap();
        ...
        Rule::odu_call => {
            Ok(Expression::OduCall(parse_odu_call(pair)?))
        }
        ...
        _ => Err(IfaError::Parse(format!("Unexpected rule: {:?}", pair.as_rule()))),
    }
}
...
fn parse_odu_call(pair: pest::iterators::Pair<Rule>) -> IfaResult<OduCall> {
    ...
    Ok(OduCall { domain, method, args, span })
}

fn parse_odu_domain(s: &str) -> IfaResult<OduDomain> {
    let lower = s.to_lowercase()
        .replace('ọ', "o").replace('ẹ', "e")
        .replace('ṣ', "s").replace(['à', 'á'], "a")
        .replace(['è', 'é'], "e").replace(['ì', 'í'], "i")
        .replace(['ò', 'ó'], "o").replace(['ù', 'ú'], "u");
...
fn parse_binary_op(pair: &pest::iterators::Pair<Rule>) -> IfaResult<BinaryOperator> {
    match pair.as_rule() {
        ...
        _ => Err(IfaError::Parse(format!("Unknown operator: {:?}", pair.as_rule()))),
    }
}
