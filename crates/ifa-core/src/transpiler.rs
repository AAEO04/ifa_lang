...
impl RustTranspiler {
    ...
    fn transpile_program(&mut self, program: &Program) -> String {
        ...
        format!(r#"//! Generated by Ifá-Lang Transpiler
//! Àṣẹ - May it be so
...
"#)
    ...
    fn transpile_statement(&mut self, stmt: &Statement) -> String {
        ...
        match stmt {
            ...
            Statement::EseDef { name, params, body, visibility, .. } => {
                ...
                let params_str: Vec<String> = params.iter()
                    ...
                    .collect();
                let mut result = format!("{}{}fn {}({}) -> IfaValue {{\n", 
                    indent, vis, name, params_str.join(", "));
            ...
            Statement::OduDef { name, body, visibility, .. } => {
                let vis = match visibility {
                    Visibility::Public => "pub ",
                    _ => "",
                };
    ...
    fn transpile_odu_call(&self, call: &OduCall) -> String {
        ...
        match (domain_str.as_str(), method.as_str()) {
            ...
            ("iwori", "duro") | ("iwori", "sleep") => {
                if let Some(arg) = args.first() {
                    format!("std::thread::sleep(std::time::Duration::from_millis({} as u64))", arg)
                } else {
        ...
        match expr {
            ...
            Expression::List(items) => {
                let items_str: Vec<String> = items.iter()
                    .map(|i| self.transpile_expression(i))
                    .collect();
