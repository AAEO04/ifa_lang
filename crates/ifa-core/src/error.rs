...
impl IfaError {
    ...
    pub fn proverb(&self) -> &'static str {
        match self {
            ...
            IfaError::TypeError { .. } => 
                "Kì í ṣe gbogbo ẹyẹ ló ń fò. (Not all birds can fly.)",
...
impl std::fmt::Display for SpannedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if let Some(ref source) = self.source_line {
            ...
            writeln!(f, "  {} | {}^", " ".repeat(self.line.to_string().len()), " ".repeat(self.column.saturating_sub(1)))?;
        }
...
pub fn format_error(error: &IfaError, file: &str, line: usize, col: usize, source: Option<&str>) -> String {
    let mut result = format!("ERROR at {}:{}:{}: {}\n", file, line, col, error);
    
    if let Some(src) = source {
        result.push_str(&format!("  {} | {}\n", line, src));
        result.push_str(&format!("  {} | {}^\n", " ".repeat(line.to_string().len()), " ".repeat(col.saturating_sub(1))));
    }
