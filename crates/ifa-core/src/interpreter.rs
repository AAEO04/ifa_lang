...
impl Interpreter {
    ...
    fn check_capability(&self, cap: &Ofun) -> IfaResult<()> {
        if self.capabilities.check(cap) {
            ...
            Err(IfaError::PermissionDenied(format!("Capability denied: {:?}", cap)))
        }
    }
    ...
    fn import_module(&mut self, path: &[String]) -> IfaResult<()> {
        ...
        let source = std::fs::read_to_string(&file_path)
            .map_err(|e| IfaError::Runtime(format!("Cannot read module '{}': {}", module_key, e)))?;
        
        let program = crate::parser::parse(&source)
            .map_err(|e| IfaError::Runtime(format!("Parse error in module '{}': {}", module_key, e)))?;
    ...
    fn execute_statement(&mut self, stmt: &Statement) -> IfaResult<IfaValue> {
        match stmt {
            ...
            Statement::Assignment { target, value, .. } => {
                ...
                match target {
                    ...
                    AssignTarget::Index { name, index } => {
                        ...
                        let container = self.env.get(name)
                        ...
                        match (container, idx) {
                            (IfaValue::List(mut list), IfaValue::Int(i)) => {
                                ...
                                if idx < list.len() {
                                    ...
                                    return Err(IfaError::Runtime(format!("Index {} out of bounds", i)));
                                }
                            }
            ...
            Statement::Ewo { condition, message, span: _ } => {
                ...
                match result {
                    ...
                    other => {
                        ...
                        Err(IfaError::Runtime(format!(
                            "[ẹ̀wọ̀] Assertion expects boolean, got: {:?}", other
                        )))
                    }
                }
            }
            ...
            Statement::Opon { size, .. } => {
                ...
                if let Some(opon_size) = OponSize::from_str(size) {
                    println!("[opon] Memory configured: {} ({} slots, {})", 
                        opon_size.display_name(), opon_size.slot_count(), opon_size.approx_memory());
                } else {
                    println!("[opon] Warning: Unknown size '{}', using default (arinrin)", size);
                }
                Ok(IfaValue::Null)
            }
        }
    ...
    fn execute_odu_call(&mut self, call: &OduCall) -> IfaResult<IfaValue> {
        ...
        match call.domain {
            ...
            OduDomain::Irosu => match call.method.as_str() {
                ...
                _ => Err(IfaError::Runtime(format!("Unknown Ìrosù method: {}", call.method))),
            },
            ...
            OduDomain::Obara => match call.method.as_str() {
                ...
                _ => Err(IfaError::Runtime(format!("Unknown Ọ̀bàrà method: {}", call.method))),
            },
            ...
            OduDomain::Oturupon => match call.method.as_str() {
                ...
                _ => Err(IfaError::Runtime(format!("Unknown Òtúúrúpọ̀n method: {}", call.method))),
            },
            ...
            OduDomain::Ika => match call.method.as_str() {
                ...
                "pin" | "split" => {
                    if args.len() >= 2 {
                        if let (IfaValue::Str(s), IfaValue::Str(delim)) = (&args[0], &args[1]) {
                            let parts: Vec<IfaValue> = s.split(delim.as_str())
                ...
                "ni" | "contains" => {
                    ...
                    Err(IfaError::Runtime("contains requires string and substring".into()))
                }
                "rọpo" | "replace" => {
                    if args.len() >= 3 {
                    ...
                    Err(IfaError::Runtime("replace requires string, from, and to".into()))
                }
                "bẹrẹ_pẹlu" | "starts_with" => {
                    ...
                    Err(IfaError::Runtime("starts_with requires string and prefix".into()))
                }
                "pari_pẹlu" | "ends_with" => {
                    if args.len() >= 2 {
                    ...
                    Err(IfaError::Runtime("ends_with requires string and suffix".into()))
                }
                _ => Err(IfaError::Runtime(format!("Unknown Ìká method: {}", call.method))),
            },
            ...
            OduDomain::Oyeku => match call.method.as_str() {
                "jade" | "exit" => {
                    let code = args.first()
                        .and_then(|v| if let IfaValue::Int(n) = v { Some(*n as i32) } else { None })
                        .unwrap_or(0);
                ...
                _ => Err(IfaError::Runtime(format!("Unknown Ọ̀yẹ̀kú method: {}", call.method))),
            },
            ...
            OduDomain::Owonrin => match call.method.as_str() {
                ...
                _ => Err(IfaError::Runtime(format!("Unknown Ọ̀wọ́nrín method: {}", call.method))),
            },
            ...
            OduDomain::Ogunda => match call.method.as_str() {
                ...
                _ => Err(IfaError::Runtime(format!("Unknown Ògúndá method: {}", call.method))),
            },
            ...
            OduDomain::Ogbe => match call.method.as_str() {
                "awọn_asẹ" | "args" => {
                    let args_list: Vec<IfaValue> = std::env::args()
                        .skip(1)
                        .map(IfaValue::Str)
                        .collect();
                ...
                "ayika" | "env" => {
                    if let Some(IfaValue::Str(key)) = args.first() {
                        self.check_capability(&Ofun::Environment { keys: vec![key.clone()] })?;
                ...
                "ẹya" | "platform" => {
                    Ok(IfaValue::Str(std::env::consts::OS.to_string()))
                }
                _ => Err(IfaError::Runtime(format!("Unknown Ọ̀gbè method: {}", call.method))),
            },
            ...
            OduDomain::Iwori => match call.method.as_str() {
                ...
                _ => Err(IfaError::Runtime(format!("Unknown Ìwòrì method: {}", call.method))),
            },
            ...
            OduDomain::Otura => match call.method.as_str() {
                "http_get" | "gba" => {
                    if let Some(IfaValue::Str(url)) = args.first() {
                        let domain = url.split("://").nth(1).and_then(|s| s.split('/').next()).unwrap_or("").to_string();
                        self.check_capability(&Ofun::Network { domains: vec![domain] })?;
                        println!("[HTTP] GET {}", url);
                        // Placeholder - would use reqwest
                        Ok(IfaValue::Str(format!("{{\"url\": \"{}\", \"status\": 200}}", url)))
                    } else {
                ...
                "http_post" | "fi" => {
                    if args.len() >= 2 {
                        if let (IfaValue::Str(url), body) = (&args[0], &args[1]) {
                            let domain = url.split("://").nth(1).and_then(|s| s.split('/').next()).unwrap_or("").to_string();
                            self.check_capability(&Ofun::Network { domains: vec![domain.clone()] })?;
                ...
                "serve" | "sin" => {
                    if let Some(IfaValue::Int(port)) = args.first() {
                        self.check_capability(&Ofun::Network { domains: vec!["*".to_string()] })?;
                ...
                _ => Err(IfaError::Runtime(format!("Unknown Òtúrá method: {}", call.method))),
            },
            ...
            OduDomain::Odi => match call.method.as_str() {
                "ka" | "read" => {
                    if let Some(IfaValue::Str(path)) = args.first() {
                        self.check_capability(&Ofun::ReadFiles { root: std::path::PathBuf::from(path) })?;
                ...
                "ko" | "write" => {
                    if args.len() >= 2 {
                        if let (IfaValue::Str(path), content) = (&args[0], &args[1]) {
                            self.check_capability(&Ofun::WriteFiles { root: std::path::PathBuf::from(path) })?;
                ...
                _ => Err(IfaError::Runtime(format!("Unknown Òdí method: {}", call.method))),
            },
            ...
            OduDomain::Irete => match call.method.as_str() {
                "sha256" => {
                    if let Some(IfaValue::Str(data)) = args.first() {
                        ...
                        let hash = format!("{:016x}", data.bytes().fold(0u64, |h, b| h.wrapping_mul(31).wrapping_add(b as u64)));
                ...
                "uuid" => {
                    ...
                    let uuid = format!("{:08x}-{:04x}-{:04x}-{:04x}-{:012x}",
                        (now >> 96) as u32, (now >> 80) as u16, (now >> 64) as u16,
                        (now >> 48) as u16, now as u64 & 0xFFFFFFFFFFFF);
                ...
                _ => Err(IfaError::Runtime(format!("Unknown Ìrẹtẹ̀ method: {}", call.method))),
            },
            ...
            OduDomain::Osa => match call.method.as_str() {
                ...
                _ => Err(IfaError::Runtime(format!("Unknown Ọ̀sá method: {}", call.method))),
            },
            ...
            OduDomain::Ose => match call.method.as_str() {
                ...
                "nu" => {
                    let fill = args.first()
                        .and_then(|v| if let IfaValue::Str(s) = v { s.chars().next() } else { None })
                        .unwrap_or(' ');
                ...
                "ya" => {
                    ...
                    Err(IfaError::Runtime("ya requires (x: int, y: int, char: str)".into()))
                }
                "han" | "kunle" | "fihan" => {
                ...
                "tobi" => {
                    ...
                    Err(IfaError::Runtime("tobi requires (width: int, height: int)".into()))
                }
                "ila" => {
                    if args.len() >= 5 {
                        if let (IfaValue::Int(x1), IfaValue::Int(y1), IfaValue::Int(x2), IfaValue::Int(y2), IfaValue::Str(ch_str)) = 
                    ...
                    Err(IfaError::Runtime("ila requires (x1, y1, x2, y2: int, char: str)".into()))
                }
                "onigun" => {
                    if args.len() >= 5 {
                        if let (IfaValue::Int(x), IfaValue::Int(y), IfaValue::Int(w), IfaValue::Int(h), IfaValue::Str(ch_str)) = 
                               (&args[0], &args[1], &args[2], &args[3], &args[4]) {
                            let ch = ch_str.chars().next().unwrap_or('#');
                    ...
                    Err(IfaError::Runtime("onigun requires (x, y, w, h: int, char: str)".into()))
                }
                "onigun_kun" => {
                    if args.len() >= 5 {
                        if let (IfaValue::Int(x), IfaValue::Int(y), IfaValue::Int(w), IfaValue::Int(h), IfaValue::Str(ch_str)) = 
                    ...
                    Err(IfaError::Runtime("onigun_kun requires (x, y, w, h: int, char: str)".into()))
                }
                "iyokoto" => {
                    if args.len() >= 4 {
                        if let (IfaValue::Int(xc), IfaValue::Int(yc), IfaValue::Int(r), IfaValue::Str(ch_str)) = 
                    ...
                    Err(IfaError::Runtime("iyokoto requires (xc, yc, r: int, char: str)".into()))
                }
                "ko" => {
                    if args.len() >= 3 {
                    ...
                    Err(IfaError::Runtime("ko requires (x, y: int, text: str)".into()))
                }
                "fi_x" => {
                ...
                "ya_nibi" => {
                    let ch = args.first()
                        .and_then(|v| if let IfaValue::Str(s) = v { s.chars().next() } else { None })
                        .unwrap_or('#');
                    self.canvas.set_pixel(self.canvas.cursor_x as i64, self.canvas.cursor_y as i64, ch);
                    Ok(IfaValue::Null)
                }
                ...
                "box" | "apoti" | "botini" => {
                    ...
                    Err(IfaError::Runtime("box requires x, y, width, height OR botini requires x, y, label".into()))
                }
                _ => Err(IfaError::Runtime(format!("Unknown Ọ̀ṣẹ́ method: {}", call.method))),
            },
            ...
            OduDomain::Ofun => match call.method.as_str() {
                ...
                _ => Err(IfaError::Runtime(format!("Unknown Òfún method: {}", call.method))),
            },
            ...
            OduDomain::Coop => match call.method.as_str() {
                "py" => {
                    ...
                    if args.len() >= 2 {
                        ...
                        let func_args: Vec<String> = args[2..].iter().map(|a| {
                            let s = a.to_string();
                        ...
                        let script = format!("import {}; print({}.{}({}))", module, module, func, py_args);
                        ...
                        match output {
                            ...
                            Err(e) => {
                                return Err(IfaError::Runtime(format!("Failed to run Python: {}. Is Python installed?", e)));
                            }
                        }
                    }
                    Err(IfaError::Runtime("py requires module and function name".into()))
                }
                "sh" | "shell" => {
                    ...
                    if let Some(IfaValue::Str(cmd)) = args.first() {
                        ...
                        match output {
                            Ok(o) => Ok(IfaValue::Str(String::from_utf8_lossy(&o.stdout).to_string())),
                ...
                "js" | "node" => {
                    ...
                    if let Some(IfaValue::Str(code)) = args.first() {
                        ...
                        match output {
                            ...
                            Ok(o) => {
                                let err = String::from_utf8_lossy(&o.stderr).trim().to_string();
                                return Err(IfaError::Runtime(format!("JavaScript error: {}", err)));
                            }
                            Err(e) => {
                                return Err(IfaError::Runtime(format!("Failed to run Node.js: {}. Is Node installed?", e)));
                            }
                        }
                    }
                ...
                "c" | "gcc" => {
                    ...
                    if let Some(IfaValue::Str(code)) = args.first() {
                        ...
                        let exe_file = temp_dir.join(if cfg!(windows) { "ifa_temp.exe" } else { "ifa_temp" });
                        ...
                        match compile {
                            Ok(o) if o.status.success() => {
                                ...
                                match run {
                                    ...
                                    Err(e) => return Err(IfaError::Runtime(format!("Failed to run C program: {}", e))),
                                }
                            }
                            Ok(o) => {
                                let _ = std::fs::remove_file(&src_file);
                            ...
                            Err(e) => {
                                let _ = std::fs::remove_file(&src_file);
                                return Err(IfaError::Runtime(format!("Failed to run compiler: {}. Is GCC installed?", e)));
                            }
                        }
                    }
                ...
                "version" => {
                    Ok(IfaValue::Str("AjoseBridge v1.0".to_string()))
                }
                _ => Err(IfaError::Runtime(format!("Unknown Coop method: {}", call.method))),
            },
            ...
            OduDomain::Opele => match call.method.as_str() {
                "cast" | "ju" => {
                    ...
                    let names = ["Ogbe", "Oyeku", "Iwori", "Odi", "Irosu", "Owonrin", 
                    ...
                    if right == left {
                        ...
                        Ok(IfaValue::Str(format!("{}_{}", names[right as usize], names[left as usize])))
                    }
                }
                ...
                "lineage" | "iran" => {
                    ...
                    if let Some(IfaValue::Str(compound)) = args.first() {
                        ...
                        let roles = match depth {
                            ...
                            5 => vec!["Great²-Grandparent", "Great-Grandparent", "Grandparent", "Parent", "Child"],
                            _ => (0..depth).map(|i| if i == depth - 1 { "Current" } else { "Ancestor" }).collect(),
                        };
                        
                        let lineage: Vec<String> = parts.iter()
                            .zip(roles.iter())
                    ...
                    Err(IfaError::Runtime("lineage requires compound name string".into()))
                }
                "depth" | "ijinle" => {
                    ...
                    Err(IfaError::Runtime("depth requires compound name string".into()))
                }
                "divine" | "dawole" => {
                ...
                _ => Err(IfaError::Runtime(format!("Unknown Ọpẹlẹ method: {}", call.method))),
            },
            
            // Default for other domains
            _ => {
                println!("Warning: {:?}.{}() not yet implemented", call.domain, call.method);
                Ok(IfaValue::Null)
            }
        }
    }
    
    fn apply_binary_op(&self, left: &IfaValue, op: &BinaryOperator, right: &IfaValue) -> IfaResult<IfaValue> {
        match op {
    ...
    fn call_method(&mut self, _obj: &IfaValue, method: &str, _args: &[Expression]) -> IfaResult<IfaValue> {
        Err(IfaError::Runtime(format!("Method '{}' not implemented", method)))
    }
...
mod tests {
    ...
    fn test_string_concat() {
        ...
        assert_eq!(interp.env.get("s"), Some(IfaValue::Str("Hello World".to_string())));
    }
    ...
    fn test_string_upper() {
        ...
        assert_eq!(interp.env.get("s"), Some(IfaValue::Str("HELLO".to_string())));
    }
